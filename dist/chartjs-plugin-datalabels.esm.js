...
function clipped(segment, area, config) {
  var x0 = segment.x0;
  var y0 = segment.y0;
  var x1 = segment.x1;
  var y1 = segment.y1;
  const localY1 = segment.y1 - 40 < 0 ? 0 : segment.y1 - 40;
  var r0 = region(x0, y0, area);
  var r1 = region(x1, localY1, area);
  var r, x, y;
  ...
  while (true) {
    ...
    if (r & R_TOP) {
      ...
      y = area.top + config.lines.length * 8;
    } else if (r & R_BOTTOM) {
...
function compute$1(range, config) {
  ...
  if (config.clamp) {
    segment = clipped(segment, config.area, config);
  }
